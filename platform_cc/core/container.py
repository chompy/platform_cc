...
class Container:
    ...
    def _createNFSVolume(self, volumeId):
        """
        ...
        try:
            ...
            pass
        labels = Container.getLabels(self)
        path_string = ":/System/Volumes/Data" + nfs_path
        self.docker.volumes.create(
            volumeId,
            labels=labels,
            driver='local', 
            driver_opts={'type':'nfs', 'o':'addr=host.docker.internal,rw,nolock,hard,nointr,nfsvers=3','device':path_string}
        )
    ...
    def shell(self, cmd="bash", user="root", stdin=None):
        ...
        if stdin and not stdin.isatty():
            with tempfile.NamedTemporaryFile(delete=True) as stdinFile:
                try:
                    chunk = stdin.buffer.raw.read(1024)
                    while chunk:
                        stdinFile.write(chunk)
                        chunk = stdin.buffer.raw.read(1024)
                except:
